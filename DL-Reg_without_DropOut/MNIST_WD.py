'''Train CIFAR10 with PyTorch using the Weight_Decay regularizer.'''
import torch
...
net = mlp_relu(in_ch=784, num_classes=10)          # The running model is a simple MLP with a ReLu activation
...
# Training
print('Training MNIST using the Weight_Decay regularizer...')
def train(epoch):
    print('\nEpoch: %d' % epoch)
    net.train()
    train_loss = 0
...
for epoch in range(start_epoch, start_epoch+1250):
    ...
    testAcc = test(epoch)
    file1 = open("WD_MNIST_TrainAcc.txt", "a")
    file2 = open("WD_TrainLoss.txt", "a")
    file3 = open("WD_TestAcc.txt", "a")
