'''Train CIFAR10 with PyTorch using the proposed regularization method.'''
import torch
...
# Training
print('Training on CIFAR10 using the proposed regularizer...')
def train(epoch):
    print('\nEpoch: %d' % epoch)
    net.train()
    train_loss = 0
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        Z = torch.inverse(torch.transpose(inputs, 0, 1) @ inputs + torch.eye(3072).to(device)) @ torch.transpose(inputs, 0,
                                                                                                            1) @ outputs
        estim = inputs @ Z
...
for epoch in range(start_epoch, start_epoch+1250):
    ...
    testAcc = test(epoch)
    file1 = open("proposed_CIFAR10_TrainAcc.txt", "a")
    file2 = open("proposed_CIFAR10_TrainLoss.txt", "a")
    file3 = open("proposed_CIFAR10_TestAcc.txt", "a")
